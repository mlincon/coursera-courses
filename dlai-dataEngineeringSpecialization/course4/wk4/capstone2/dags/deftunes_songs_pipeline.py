...
DATA_BUCKET_NAME = "de-c4w4a2-817626893719-us-east-1-data-lake"
SCRIPTS_BUCKET_NAME = "de-c4w4a2-817626893719-us-east-1-scripts"
...
def deftunes_songs_pipeline():
    ...
    rds_extract_glue_job = GlueJobOperator(
        ...
        # The name of the Glue job you created in Terraform. This corresponds to the value that
        # you can find in the Terraform output with the key `glue_rds_extract_job`
        ...
        # The parameters depend on the job but you can find the data lake bucket name, the
        # RDS connection, and the ingestion date.
        # For the ingestion date, pay attention to the usage of Jinja templating to get the next date.
        # Currently the `"{{ next_ds }}"` variable is deprecated but you are using it by simplicity
        # to take the next start date.
        script_args={
    ...
    # This job transforms the CSV files extracted from the RDS, adds some metadata and saves the
    # information in Apache Iceberg format in the transformation zone.
    ...
    dq_check_songs_job = GlueDataQualityRuleSetEvaluationRunOperator(
        ...
        # In this case, instead of just the name, you are required to pass the complete ARN as in
        # the Terraform output `glue_role_arn`.
        role="arn:aws:iam::817626893719:role/Cloud9-de-c4w4a2-glue-role",
        # A list with the set of rules that you want to evaluate. For the `songs` data coming from
        # the RDS, the name of the rule set is in the Terraform output `songs_db_ruleset_name`.
    ...
    # This task is in charge of using DBT to take the data from the `deftunes_transform` schema created
    # with Redshift Spectrum and transform it to a star schema hosted in the `deftunes_serving` schema.
    # This operator takes a docker image to create a container and passes a command to be executed.
    task_dbt = DockerOperator(
        ...
        api_version="auto",
        # As `auto_remove` is set to `True`, the Docker container will be automatically removed after the
        # task finishes. This helps to clean up and not leave unused containers behind.
        auto_remove=True,
        # This specifies the URL to the Docker daemon.
        # In this case, `"unix://var/run/docker.sock"` is used, which is the default Unix socket for Docker
        # on most Linux systems. This URL is how Airflow connects to the Docker service running on the host machine.
        docker_url="unix://var/run/docker.sock",
        # The command to run inside the Docker container. The provided command checks the version of `dbt` and
        # then runs `dbt debug` to ensure that the `dbt` configuration is correct.
        ...
        # This sets the networking mode for the container. `"container:dbt"` specifies that the container
        # should join another container’s network.
        # In this case, it joins the network of a container named `dbt`, allowing it to communicate
        # directly with that container using localhost.
