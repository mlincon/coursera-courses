...
def deftunes_pipeline():
    ...
    # Use a `GlueJobOperator` to create the `api_users_extract_glue_job` task. This task is in charge of
    # creating a Glue job that will ingest the Users endpoint
    ...
    # This task will take the data in JSON format from the users and sessions endpoints and will
    # add some metadata and perform unnest transformations.
    ...
    # This task will evaluate the rule set for users that you created with Terraform and associated
    # with the Terraform output `users_db_ruleset_name`.
