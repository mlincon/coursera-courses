...
users_df = users_df.withColumn("ingest_on", to_date(lit(ingest_date_str), "yyyy-MM-dd"))
...
users_df = users_df.withColumn("processing_timestamp", lit(processing_timestamp))
sessions_df = sessions_df.withColumn("processing_timestamp", lit(processing_timestamp))

sessions_df = sessions_df.withColumn(
    "session_ts", udf_to_timestamp(col("session_start_time"))
)
...
if users_table_exists:
    ...
    users_df.writeTo(
        f"glue_catalog.{catalog_database}.{users_table_name}"
    ).tableProperty("format-version", "2").partitionedBy("ingest_on").createOrReplace()
