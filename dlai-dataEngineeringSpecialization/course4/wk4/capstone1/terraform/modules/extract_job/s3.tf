# Complete the `data "aws_s3_bucket" "data_lake"` resource by setting the `bucket`
# parameter to `var.data_lake_name`
...
# In the resource `"aws_s3_object" "glue_job_extract_db"`, set the `bucket` to the scripts
# bucket with `aws_s3_bucket.scripts.id`
...
resource "aws_s3_object" "glue_job_extract_api" {
  bucket = aws_s3_bucket.scripts.id
  key    = "de-c4w4a1-api-extract-job.py.py"
